...
class MongoDB:
    ...
    def create_notebook(self, user_id, name, description="", color="#1E88E5"):
        """Create a new notebook for the user."""
        if self.client is None:
            return False, "Database connection not established"
            
        try:
            # Create the notebook document
            notebook = {
                "name": name,
            ...
            # Insert into notebooks collection
            result = self.db.notebooks.insert_one(notebook)
            notebook_id = str(result.inserted_id)
            
            # Add reference to user's notebooks
            self.db.users.update_one(
                {"_id": ObjectId(user_id)},
                {"$push": {"notebooks": ObjectId(notebook_id)}}
            )
